CREATE (ic:ConsistencyCheck { ccid:1,
		ccname: "DTP_DOMAIN",
		cccypher: "MATCH (n:Class)<-[:DOMAIN]-(p:DatatypeProperty) 
				...
				RETURN  id(x) AS nodeUID, 
        			'violation of consistency check on domain of property ' + propLabel + ' [' + prop + ']' AS check,  
					'Node labels: (' + reduce(s = '', l IN Labels(x) | s + ' ' + l) + ') should include ' + classLabel AS extraInfo"})
		
CREATE (ic:ConsistencyCheck { ccid:2,
		ccname: "OP_DOMAIN",
		cccypher: "MATCH (n:Class)<-[:DOMAIN]-(p:ObjectProperty) 
			...
			RETURN  id(x) AS nodeUID, 
        		'violation of consistency check on domain of ' + propLabel + ' [' + prop + ']' AS check,  
				'Node labels: (' + reduce(s = '', l IN Labels(x) | s + ' ' + l) + ') should include ' + classLabel AS extraInfo	"})
		
CREATE (ic:ConsistencyCheck { ccid:3,
		ccname: "OP_RANGE",
...
CREATE (ic:ConsistencyCheck { ccid:4,
